...
app.post("/login", async (req, res) => {
  ...
  try {
    ...
    const decodedToken = jwt.decode(token) as { sub: string; email?: string; name?: string }; // Explicitly typing the decoded token
...
// Fetch all documents for a specific user by googleId


// In your route handler, modify the query to convert googleId to ObjectId


// In your route handler, modify the query to convert googleId to ObjectId

 // Adjust according to your file model import
...
app.get('/documents', async (req, res) => {
  try {
    ...
    const documents = await FileModel.find({ userId: googleId }, "title _id parsedResponse");
...
app.get('/documents/:id',  authenticateToken ,async (req, res) => {
  try {
    ...
    const document = await FileModel.findOne({ _id: id, userId: googleId }); // Ensure the document belongs to the user
